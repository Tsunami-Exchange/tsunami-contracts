...
const isMigrated = async(seq) => {
    ...
    const last = await accountDataByKey(key, masterAddress).then(x => (x && Number.parseInt(x.value)) || 0)
    return last >= seq
}
...
const run = async() => {
    ...
    for (let migration of migrations) {
        ...
        if (!isRun) {
            ...
            console.log(`Migrations cost: ${(balanceBefore - balanceAfter) / 10**8} WAVES`)
