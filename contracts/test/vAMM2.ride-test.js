...
async function getPositionActualMargin(trader) {
    ...
    try {
        ...
        console.log(`fundingPaymen=${parseInt(parts[1])}`)
        return margin
    }
}
...
describe('vAMM should work with positive fundind', async function () {
    ...
    before(async function () {
        ...
        let p3 = deploy('vAMM2.ride'       , 4500000, accounts.wallet      , 'vAMM')
    ...
    it('Can decrease short position', async function () {
        ...
        expect(changes[`k_positionMargin_${address(accounts.shorter)}`]).to.be.eq(4949996) // 5 USDN - 1%
        expect(changes[`k_positionOpenNotional_${address(accounts.shorter)}`]).to.be.eq(11850004) // 3 * (5 USDN - 1%) 
...
describe('vAMM should be able to liquidate underwater position', async function () {
    ...
    before(async function () {
        ...
        let p1 = deploy('coordinator.ride' , 3400000, accounts.coordinator , 'Coordinator')
        let p2 = deploy('insurance.ride'   , 3400000, accounts.insurance   , 'Insurance Fund')
        let p3 = deploy('vAMM2.ride'       , 4100000, accounts.wallet      , 'vAMM')
