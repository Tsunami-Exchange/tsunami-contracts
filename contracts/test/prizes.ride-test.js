...
const createTokenSignature = (_signer, _nonce, _recipient, _assetId, _amount) => {
    ...
    const r = [[_nonce, _recipient, assetStr, _amount], signBytes(_signer, new TextEncoder().encode(prizeStr))]
    return r
}

const createMintTokenSignature = (_signer, _nonce, _recipient, _assetId, _amount) => {
...
const createMultiTokenSignature = (_signer, _nonce, _recipient, _assetIds, _amounts) => {
    ...
    const r = [[_nonce, _recipient, assetStr, amountsStr], signBytes(_signer, new TextEncoder().encode(prizeStr))]
    return r
}
...
describe('Prize Manager should be able', async function () {
    ...
    it('to send token multiple rewards to user with correct signature', async function () {
        let [data, signature] = createMultiTokenSignature(e.seeds.admin, uuidv4(), address(user2), [e.assets.tsn, e.assets.neutrino], [1 * wvs, 2 * decimals])
    ...
    it('mint single existing nft type', async function () {
        let [data, signature] = createMintTokenSignature(e.seeds.admin, uuidv4(), address(user), "fee_reduction", 1)
    ...
    it('mint multiple existing nft type', async function () {
        let [data, signature] = createMintTokenSignature(e.seeds.admin, uuidv4(), address(user), "fee_reduction", 2)
        let tx = await e.prizes.as(user).claimPrize(...data, signature)
    ...
    it('allow attaching NFT to make new positions', async function () {
        let tx = await amm.as(user).increasePosition(100, DIR_LONG, 1, 0, "", artifactId)
