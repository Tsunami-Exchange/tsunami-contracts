...
describe("Miner should compute and distribute m rewards", async function () {
  ...
  it("Should compute average notional over period", async function () {
    ...
    let t1Notional = await e.miner.getTraderNotionalInPeriod(
      amm,
      trader1,
      period
    );
  ...
  it("Should compute average notional over period with gaps", async function () {
    ...
    expect(t1Notional / wvs).to.be.closeTo(
      (1 / 7) * 0 + (1 / 7) * 1000 + (5 / 7) * 0,
      0.01
    );
  });
  ...
  it("Should compute average notional for second trader", async function () {
    ...
    let t2Notional = await e.miner.getTraderNotionalInPeriod(
      amm,
      trader2,
      period
    );
  ...
  it("Should distribute rewards according to price", async function () {
    ...
    const { rewards: t1Reward } = await e.miner.getTraderRewardInPeriod(
      e.assets.tsn,
      trader1,
      period
    );
    const { rewards: t2Reward } = await e.miner.getTraderRewardInPeriod(
      e.assets.tsn,
      trader2,
      period
    );
  ...
  it("Should not allow trader to claim rewards for current period", async function () {
    ...
    expect(e.miner.as(trader1pk).claimAllRewards(e.assets.tsn, period)).to
  ...
  it("Should allow trader to claim rewards in next period", async function () {
    ...
    const { rewards, claimed } = await e.miner.getTraderRewardInPeriod(
      e.assets.tsn,
      trader1,
      period
    );
