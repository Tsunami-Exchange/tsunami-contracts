...
const createTakeProfitOrderAndSignature = (_trader, _amm, _price, _due) => {
  const orderStr = `1,${_amm},${publicKey(_trader)},${Math.round(
    _price * decimals
  )},${new Date().getTime()},${_due || 0}`;
  const r = [
    "123",
    orderStr,
    signBytes(_trader, new TextEncoder().encode("123" + orderStr)),
  ];
  return r;
};

const createStopLossOrderAndSignature = (_trader, _amm, _price, _due) => {
  const orderStr = `2,${_amm},${publicKey(_trader)},${Math.round(
    _price * decimals
  )},${new Date().getTime()},${_due || 0}`;
  const r = [
    "123",
...
describe("vAMM should be able to execute delayed take profit orders", async function () {
  ...
  it("Rejects order when price has not moved", async function () {
    const [prefix, order, sig] = createTakeProfitOrderAndSignature(
      longer,
      amm.address,
      60.0
    );
    ...
    return await expect(e.orders.executeOrder(prefix, order, sig)).to.be
      .eventually.rejected;
  });
...
describe("vAMM should be able to execute delayed stop loss orders", async function () {
  ...
  it("Rejects order when price has not moved", async function () {
    const [prefix, order, sig] = createStopLossOrderAndSignature(
      shorter,
      amm.address,
      50.0
    );
