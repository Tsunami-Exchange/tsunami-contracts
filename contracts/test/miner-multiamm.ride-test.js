...
describe('Miner should work with multiple AMMs and same reward token', async function () {
    ...
    it('Should take relative fees on AMM into account when computing max distributed amount', async function () {
        ...
        let max1 = await e.miner.getMaxAmountOfAssetToDistribute(amm1, e.assets.tsn, period)
        let max2 = await e.miner.getMaxAmountOfAssetToDistribute(amm2, e.assets.tsn, period)
        ...
        await Promise.all([
            e.miner.getTraderRewardInPeriod(e.assets.tsn, trader1, period).then(r => rewards[trader1] = r.rewards),
            e.miner.getTraderRewardInPeriod(e.assets.tsn, trader2, period).then(r => rewards[trader2] = r.rewards),
            e.miner.getTraderRewardInPeriod(e.assets.tsn, trader3, period).then(r => rewards[trader3] = r.rewards),
            e.miner.getTraderRewardInPeriod(e.assets.tsn, trader4, period).then(r => rewards[trader4] = r.rewards)
        ])

        const sum = rewards[trader1] + rewards[trader2] + rewards[trader3] + rewards[trader4]
