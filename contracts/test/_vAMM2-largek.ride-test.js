...
describe("vAMM should work with large K", async function () {
  this.timeout(600000);
  ...
  it("Can open position", async function () {
    await amm.as(longer).increasePosition(30000, DIR_LONG, 3, 0.15);

    const { size, margin, openNotional } = await amm.getPositionInfo(longer);

    expect(size).to.be.eq(1605693273);

    const { totalSize, totalLong, totalShort } = await amm.totalPositionInfo();

    expect(totalSize).to.be.eq(1605693273);
    expect(totalLong).to.be.eq(1605693273);
