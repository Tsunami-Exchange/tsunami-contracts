...
describe("Staking should allow (with other quote asset)", async function () {
  this.timeout(600000);

  let e, c, staker1, staker2, rewardPayer;
  ...
  before(async function () {
    ...
    await e.deploy();
    c = await e.deployDefaultChild(e.assets.usdt);
  ...
  it("Can add rewards to Staking", async function () {
    await e.fundAccounts({ [c.seeds.staking]: 1000 });
    let tx1 = await c.staking.as(rewardPayer).ackRewards(); // on child
    console.log(`Ack reward (child) in ${tx1.id}`);
    let tx2 = await e.staking.as(rewardPayer).ackRewards(); // on parent
    console.log(`Ack reward (parent) in ${tx2.id}`);
  });
