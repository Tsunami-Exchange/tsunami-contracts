...
function updateReserve(_isAdd, _quoteAssetAmount, _baseAssetAmount) {
    ...
    if (_isAdd) {
        ...
        baseAssetDeltaThisFundingPeriodAfter = baseAssetDeltaThisFundingPeriod - _baseAssetAmount;
...
function swapInput(_isAdd, _quoteAssetAmoun) {
    ...
    const amountBaseAssetBoughtAbs = Math.abs(baseAssetReserveAfter - baseAssetReserveBefore)
...
function removeMargin(_investor, _removedMargin) {
    ...
    const [remainMargin, badDebt, _, latestCumulativePremiumFraction] = calcRemainMarginWithFundingPayment(
        position.size,
        position.margin,
    ...
    positions[_investor].lastUpdatedCumulativePremiumFraction = latestCumulativePremiumFraction
...
function mock_getPositionBadDebtAndFundingPayment(_investor) {
    ...
    const [__, badDebt, fundingPayment, ___] = 
...
function calcRemainMarginWithFundingPayment(_oldPositionSize, _oldPositionMargin, _oldPositionLastUpdatedCumulativePremiumFraction, _marginDelta) {
    // calculate funding payment
    //
    var fundingPayment = 0
    if (_oldPositionSize != 0) {
        fundingPayment = muld((latestCumulativePremiumFraction - 
            _oldPositionLastUpdatedCumulativePremiumFraction), _oldPositionSize)
    }
    ...
    return [remainMargin, badDebt, fundingPayment, latestCumulativePremiumFraction]
}
...
function internalClosePosition(_investor) {
    ...
    const [ exchangedQuoteAssetAmount, 
        quoteAssetReserveAfter,
        baseAssetReserveAfter,
        baseAssetDeltaThisFundingPeriodAfter,
        totalPositionSizeAfter,
    ...
    const openInteresetNotionalAfter = openInteresetNotional - oldPosition.openNotional
...
function openPosition(_investor, _direction, _amount, _leverage, _minBaseAssetAmount = 0) {
    requireMoreMarginRatio(divd(unit, _leverage), initMarginRatio, true);

    const oldPosition = getPosition(_investor)
    const isNewPosition = oldPosition.size === 0
    const expandExisting = !isNewPosition && ((oldPosition.size > 0 ? DIR_LONG : DIR_SHORT) == _direction)
    ...
    if (isNewPosition || expandExisting) {
        ...
        if (_minBaseAssetAmount !== 0 && amountBaseAssetBought < _minBaseAssetAmount) {
        ...
        newPosition.lastUpdatedCumulativePremiumFraction = oldLatestCumulativePremiumFraction

        newAmmState.baseAssetReserve = baseAssetReserveAfter
        newAmmState.quouteAssetReserve = quouteAssetReserveAfter
        newAmmState.baseAssetDeltaThisFundingPeriodAfter = baseAssetDeltaThisFundingPeriodAfter
        ...
        if (oldPositionNotional > openNotional) {
            ...
            if (_minBaseAssetAmount !== 0 && Math.abs(exchangedPositionSize) < _minBaseAssetAmount) {
                throw Error('Too little basse asset bought')
            }

            let realizedPnl = 0
            if (oldPosition.size != 0) {
                realizedPnl = divd(muld(unrealizedPnl, Math.abs(exchangedPositionSize)), oldPosition.size)
            }
            
            [remainMargin, badDebt, fundingPayment, oldLatestCumulativePremiumFraction] = 
                calcRemainMarginWithFundingPayment(
                    oldPosition.size, 
                    oldPosition.margin, 
                    oldPosition.lastUpdatedCumulativePremiumFraction, 
            ...
            const remainOpenNotional = oldPosition.size > 0
    ...
    baseAssetDeltaThisFundingPeriod = newAmmState.baseAssetDeltaThisFundingPeriodAfter 
...
function getOutputPriceWithReserves(_add, _baseAssetAmount, _quoteAssetPoolAmount, _baseAssetPoolAmount) {
...
function payFunding() {
    ...
    const ammFundingPaymentProfit = muld(premiumFraction, totalTraderPositionSize)
    ...
    baseAssetDeltaThisFundingPeriod = 0
    nextFundingBlock = nextFundingBlock + 60
    latestCumulativePremiumFraction = latestCumulativePremiumFraction + premiumFraction
...
function getMarginRatio(_trader) {
    ...
    const [positionNotional, unrealizedPnl] = getPositionNotionalAndUnrealizedPnl(_trader)
...
function liquidate(_liquidator, _investor) {
    requireMoreMarginRatio(getMarginRatio(_investor), maintenanceMarginRatio, false)
    ...
    const feeToLiquidator = muld(exchangedQuoteAssetAmount, liquidationFeeRatio) / 2
    ...
    traderBalances[_liquidator] = traderBalances[_liquidator] + Math.abs(feeToLiquidator)
...
function requireMoreMarginRatio(_marginRatio, _baseMarginRatio, _largerThanOrEqualTo) {
...
function getPersonalPositionWithFundingPayment(_trader) {
    const position = getPosition(_trader)
    const marginWithFundingPayment = position.margin + getFundingPayment(position, latestCumulativePremiumFraction)
    return {
        ...position,
...
function getFundingPayment(position, latestCumulativePremiumFraction) {
    if (position.size === 0) {
        ...
        const cumulativePremiumFractionDelta = latestCumulativePremiumFraction - position.lastUpdatedCumulativePremiumFraction
        const positionSizeDeltaFraction = muld(cumulativePremiumFractionDelta, position.size)
...
function debug_calcRemainMarginWithFundingPayment(_trader) {
  ...
  let positionLastUpdatedCumulativePremiumFraction = position.lastUpdatedCumulativePremiumFraction
  ...
  throw(
    ...
    + " positionLastUpdatedCumulativePremiumFraction=" + positionLastUpdatedCumulativePremiumFraction.toString()
    + " positionNotional=" + positionNotional.toString()
    + " unrealizedPnl=" + unrealizedPnl.toString()
    + " remainMargin=" + remainMargin.toString()
